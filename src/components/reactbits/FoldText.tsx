...
const FoldText: React.FC<FoldTextProps> = ({
  ...
  const segments = useMemo(() => {
    ...
    const renderSegment = (content: string, key: string, split: string = splitBy, isSpace: boolean = false) => {
      segmentIndex += 1;
      return (
        <span
        ...
        >
          <span
            ...
            data-fold-space={isSpace || undefined}
    ...
    // Group each word's character-pieces inside a `white-space: nowrap`
    // wrapper so the line can only break at real word boundaries, not
    // between two letters of the same word (adjacent inline-block spans
    // otherwise expose a break opportunity at every character).
    const lines = text.split('\n');
    return lines.flatMap((line, lineIndex) => {
      const words = line.split(/(\s+)/).flatMap((token, tokenIndex) => {
        if (!token) return [];
        if (/^\s+$/.test(token)) {
          return Array.from(token).map((_, i) =>
            renderSegment('\u00A0', `ws-${lineIndex}-${tokenIndex}-${i}`, splitBy, true)
          );
        }
        return (
          <span className="fold-text-word" key={`word-${lineIndex}-${tokenIndex}`}>
            {Array.from(token).map((char, i) => renderSegment(char, `char-${lineIndex}-${tokenIndex}-${i}`))}
          </span>
        );
      });
      return lineIndex < lines.length - 1 ? [...words, <br key={`br-${lineIndex}`} />] : words;
    });
  ...
  const rootStyle = {
    ...
    ...(letterSpacing !== undefined ? { '--fold-text-letter-spacing': letterSpacing } : {}),
    ...(wordSpacing !== undefined ? { '--fold-text-word-spacing': wordSpacing } : {}),
